...
def route_page(Type_path=None):
	if not ('username' in session): 
		...
		if Type_path == "addacc":
				return render_template('admin_add_account.html' ,ret2=" ", ret='successfully')
		if Type_path == "addacc2":
				data=User.query.all()
			...
			try:
				...
				if 'insert' in typ:
					...
					if le_el == '1234':
						...
						level = 'زائر'					
					us=User(username=username , password=generate_password_hash(PassWord(data)), department=department ,level=level, le_el=le_el)
					db.session.add(us)
					db.session.commit()
					return render_template('admin_add_account.html' ,ret2="", ret='successfully')
				elif 'upload' in typ :
					...
					return render_template('admin_add_account.html',ret2=data , ret='successfully')
			except Exception as e:
				return render_template('admin_add_account.html' ,ret2="", e=e)
		return redirect(url_for('login'))
	return redirect(url_for('login'))
